...
export const ProductList = () => {
  ...
  useEffect(() => {
    if (prevCartPhonesArr) {
      localStorage.setItem(
        'savedCartName',
        JSON.stringify(prevCartPhonesArr),
      );
    }
  }, [prevCartPhonesArr]);
    ...
    if (cartPhones.trim() !== '') {
      if (prevCartPhonesArr?.some(elem => elem.id === cartPhones)) {
        setPrevCartPhonesArr(
          prevCartPhonesArr?.filter(phone => phone.id !== cartPhones),
        );
      } else {
        setPrevCartPhonesArr(
          prevCartPhonesArr
  ...
  useEffect(() => {
    if (favoritePhones.trim() !== '') {
      if (prevFavoriteArr?.includes(favoritePhones)) {
        setPrevFavoriteArr(
          prevFavoriteArr?.filter(phone => phone !== favoritePhones),
        );
      } else {
        setPrevFavoriteArr(
          (prevFavoriteArr
  ...
  useEffect(() => {
    if (getPhone) {
      ...
      if (sortBy === 'Newest') {
        sortedPhones = sortedPhones.sort(
          (phoneA, phoneB) => phoneB.year - phoneA.year,
        );
      }

      if (sortBy === 'Alphabetically') {
        sortedPhones = sortedPhones.sort(
          (phoneA, phoneB) => phoneA.name.localeCompare(phoneB.name),
        );
      }

      if (sortBy === 'Cheapest') {
        sortedPhones = sortedPhones.sort(
          (phoneA, phoneB) => phoneA.price - phoneB.price,
        );
      }
          ...
          {loading && (
            <div className="clockLoader__wrapper">
              <ClockLoader
                size={40}
                loading={loading}
                className="clockLoader__content"
              />
            </div>
          )}
          {!loading && (
                ...
                {!query && <div className="phones__header__buttons">
                  <div className="phones__header__buttons__sort" ref={blockSortRef}>
                    ...
                    <button className="phones__header__buttons__sort__button" onClick={handleChangeSort}>
                      ...
                      <img 
                        className="phones__header__buttons__sort__button__img" 
                        src={changeSort ? ArrowUp : ArrowDown} 
                        alt=""
                      />
                    </button>
                    {changeSort
                  ...
                  <div className="phones__header__buttons__amount" ref={blockItemsRef}>
                    ...
                    <button className="phones__header__buttons__amount__button" onClick={handleChangeItems}>
                      <span className="phones__header__buttons__amount__button__text">
                        {itemsOnPage !== getPhone?.length ? itemsOnPage : "all"}
                      </span>
                      <img
                        className="phones__header__buttons__amount__button__img"
                        src={changeItemsOnPage ? ArrowUp : ArrowDown}
                        alt="arrow"
                      />
                    </button>
                    {changeItemsOnPage
                    ...
                    <div className="hot-prices__goods__cards__good-card">
                      ...
                      <div className="hot-prices__goods__cards__good-card__header">
                        ...
                        <div className="hot-prices__goods__cards__good-card__buttons">
                          ...
                          <button
                            type="button"
                            className={prevCartPhonesArr && prevCartPhonesArr.some(elem => elem.id === phone.itemId)
                          ...
                          >
                            {prevCartPhonesArr && prevCartPhonesArr.some(elem => elem.id === phone.itemId)
                              ? 'Added to cart'
                              : 'Add to cart'
                            }
                          </button>
                          <button
                            ...
                            style={prevFavoriteArr && prevFavoriteArr.includes(phone.itemId)
